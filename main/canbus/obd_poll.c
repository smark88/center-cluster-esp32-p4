...
{
    bind_targets();
    ESP_LOGI(TAG, "polling %d PIDs every %d ms (transmitting on 0x%03X)",
             (int)PID_COUNT, OBD_POLL_INTERVAL_MS, OBD_REQ_ID);

    while (1) {
        send_request(s_pids[s_next].pid);
        s_next = (s_next + 1) % PID_COUNT;
