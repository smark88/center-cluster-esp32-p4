...
void boot_screen_create(void)
{
    ...
    lv_img_set_src(boot_logo, &corvette_c5r_yellow_flipped_720);
    ...
    uint32_t zoom_w = ((uint32_t)disp_w * LV_IMG_ZOOM_NONE) / corvette_c5r_yellow_flipped_720.header.w;
    uint32_t zoom_h = ((uint32_t)disp_h * LV_IMG_ZOOM_NONE) / corvette_c5r_yellow_flipped_720.header.h;
    uint32_t zoom = (zoom_w > zoom_h) ? zoom_w : zoom_h;   /* max ratio => cover */
    lv_img_set_pivot(boot_logo, corvette_c5r_yellow_flipped_720.header.w / 2, corvette_c5r_yellow_flipped_720.header.h / 2);
