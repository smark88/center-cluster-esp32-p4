...
void boot_screen_create(void)
{
    ...
    lv_img_set_src(boot_logo, &ui_img_c5r);
    ...
    uint32_t zoom_w = ((uint32_t)disp_w * LV_IMG_ZOOM_NONE) / ui_img_c5r.header.w;
    uint32_t zoom_h = ((uint32_t)disp_h * LV_IMG_ZOOM_NONE) / ui_img_c5r.header.h;
    uint32_t zoom = (zoom_w > zoom_h) ? zoom_w : zoom_h;   /* max ratio => cover */
    lv_img_set_pivot(boot_logo, ui_img_c5r.header.w / 2, ui_img_c5r.header.h / 2);
